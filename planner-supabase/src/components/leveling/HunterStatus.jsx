...
export default function HunterStatus() {
  ...
  if (loading || !stats) {
    return <div className="p-4 mx-4 mb-4 bg-surface-warm rounded-md animate-pulse h-24"></div>
  }
  ...
  return (
    <div className="mx-4 mb-4 p-4 bg-slate-900 rounded-xl border border-slate-700 shadow-lg relative overflow-hidden group">
      {/* Background Effect */}
      <div className="absolute top-0 right-0 -mr-4 -mt-4 w-20 h-20 bg-hunter-accent rounded-full blur-3xl opacity-20 group-hover:opacity-40 transition-opacity"></div>

      <div className="relative z-10 flex items-center justify-between mb-3">
        <div>
          <h4 className="text-white font-bold tracking-wider flex items-center gap-1.5">
            <Swords size={16} className="text-hunter-accent" />
            Lv. {stats.level}
          </h4>
        ...
        <div className="text-right">
          ...
          <p className="text-sm font-mono text-hunter-accent font-bold">{stats.total_exp.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="relative pt-1">
        <div className="flex mb-1 items-center justify-between">
          <div>
            <span className="text-[10px] font-semibold inline-block text-hunter-accent uppercase">
              EXP
            </span>
          </div>
          <div className="text-right">
        ...
        <div className="overflow-hidden h-2 mb-1 text-xs flex rounded-full bg-slate-800 border border-slate-700">
          <div
            ...
            className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-gradient-to-r from-hunter-accent to-hunter-gold transition-all duration-1000 ease-out relative"
          >
