  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 md:py-8 space-y-6 md:space-y-8">
      ...
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-fg tracking-tight">กระเป๋าเงินร่วม</h1>
          <p className="text-muted text-lg mt-1">Shared Financial View</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setShowBatchUploader(true)}
            className="flex items-center gap-2 bg-surface border border-border text-fg-2 px-4 py-2 rounded-lg font-medium hover:bg-gray-50 transition-colors"
          >
            <QrCode size={18} />
      ...
      <div className="grid grid-cols-3 gap-3 md:gap-4">
        <div className="bg-surface rounded-md border border-border p-4">
          ...
          <p className={`text-xl md:text-2xl font-bold ${balance < 0 ? 'text-red-500' : 'text-fg'}`}>
      ...
      {/* Transactions List */}
      <section>
        <h2 className="text-lg font-semibold text-fg mb-3">รายการล่าสุด</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-surface rounded-md border border-border">
            <div className="w-12 h-12 bg-gray-50 text-meta rounded-full flex items-center justify-center mx-auto mb-3">
              <ArrowRightLeft size={24} />
            </div>
            <p className="text-muted">ยังไม่มีรายการบันทึกบัญชี</p>
          </div>
        ) : (
          <div className="bg-surface rounded-md border border-border divide-y divide-gray-100">
            {transactions.map(t => (
              <div key={t.id} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-medium text-fg">{t.category}</p>
                    <p className="text-sm text-muted">{t.note || t.date}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className={`font-semibold ${t.type === 'Income' ? 'text-green-600' : 'text-fg'}`}>
                    {t.type === 'Income' ? '+' : '-'}฿{t.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
                  <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEditClick(t)} className="p-1.5 text-meta hover:text-accent rounded-lg hover:bg-accent-soft transition-colors">
                      <Pencil size={16} />
                    </button>
                    <button onClick={() => handleDelete(t.id)} className="p-1.5 text-meta hover:text-red-600 rounded-lg hover:bg-red-50 transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
      ...
      {showAddForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 pb-20 md:pb-0">
          ...
          <div className="bg-surface w-full max-w-md rounded-lg border border-border shadow-xl relative animate-[fadeIn_0.2s_ease-out]">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-fg">{editingId ? 'แก้ไขรายการ' : 'บันทึกรายการใหม่'}</h2>
              <button onClick={() => setShowAddForm(false)} className="text-meta hover:text-gray-600 p-1">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              <div className="flex gap-2 p-1 bg-gray-100 rounded-lg">
                <button
                  ...
                  className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-colors ${
                    formData.type === 'Expense' ? 'bg-surface text-fg shadow-card' : 'text-muted hover:text-gray-700'
                  }`}
                >
                  รายจ่าย
                </button>
                <button
                  ...
                  className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-colors ${
                    formData.type === 'Income' ? 'bg-surface text-fg shadow-card' : 'text-muted hover:text-gray-700'
                  }`}
                >
                  รายรับ
                </button>
              ...
              <div>
                <label className="block text-sm font-medium text-fg-2 mb-1">จำนวนเงิน (บาท)</label>
                <input
                  type="number"
                  step="0.01"
                  required
              ...
              <div>
                <label className="block text-sm font-medium text-fg-2 mb-1">รายละเอียดเพิ่มเติม (ไม่บังคับ)</label>
                <input
                  type="text"
                  value={formData.note}
      ...
      {showBatchUploader && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 pb-20 md:pb-0">
          ...
          <div className="bg-surface w-full max-w-2xl rounded-lg border border-border shadow-xl relative animate-[fadeIn_0.2s_ease-out]">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-fg">นำเข้าสลิปโอนเงิน (Batch Upload)</h2>
              <button onClick={() => setShowBatchUploader(false)} className="text-meta hover:text-gray-600 p-1">
                <X size={20} />
              </button>
            </div>
